...
const mixRatios = {
  5: { cement: 1, sand: 5, aggregate: 10 },
  7.5: { cement: 1, sand: 4, aggregate: 8 },
  10: { cement: 1, sand: 3, aggregate: 6 },
  15: { cement: 1, sand: 2, aggregate: 4 },
...
calculate.addEventListener("click", () => {
  ...
  const dryVolume = prod * 1.54;

  // const selectGrade = concreteGrade;
  ...
  calculateQuantity.cementQuant(ratio.sand, ratio.aggregate, dryVolume);
  calculateQuantity.sandQuant(ratio.sand, ratio.aggregate, dryVolume);
  calculateQuantity.aggQuant(ratio.sand, ratio.aggregate, dryVolume);
});
...
const calculateQuantity = {
  cementQuant(sandRatio, aggRatio, dryVolume) {
    cement.innerText = (
      (1 / (1 + sandRatio + aggRatio)) *
      dryVolume *
      1400
    ).toFixed(3);
  },

  sandQuant(sandRatio, aggRatio, dryVolume) {
    sand.innerText = (
      (sandRatio / (1 + sandRatio + aggRatio)) *
      dryVolume *
      1600
    ).toFixed(3);
  },
  aggQuant(sandRatio, aggRatio, dryVolume) {
    aggregate.innerText = (
      (aggRatio / (1 + sandRatio + aggRatio)) *
      dryVolume *
      1500
    ).toFixed(3);
  },
